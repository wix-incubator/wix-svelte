(function (global, factory) {
	...
	class Wrapper {
	    ...
	    get_or_create_anchor(block, parent_node, parent_nodes) {
	        ...
	        if (needs_anchor) {
	            block.add_element(anchor, x `@empty()`, parent_nodes && x `@claim_text(${parent_nodes}, '')`, parent_node);
	        }
	        return anchor;
	    }
	...
	class EachBlockWrapper extends Wrapper {
		...
		`);
	        ...
	        if (needs_anchor) {
	            block.add_element(update_anchor_node, x `@empty()`, parent_nodes && x `@claim_text(${parent_nodes}, '')`, parent_node);
	        }
	        if (this.else) {
	    ...
	    render_keyed({ block, parent_node, parent_nodes, snippet, initial_anchor_node, initial_mount_node, update_anchor_node, update_mount_node }) {
	        ...
	        if (this.fragment.nodes[0].is_dom_node()) {
	            ...
	            this.block.add_element(this.block.first, x `@empty()`, parent_nodes && x `@claim_text(#nodes, '')`, null);
	        }
	        block.chunks.init.push(b `
	...
	class ElementWrapper extends Wrapper {
	    ...
	    render(block, parent_node, parent_nodes) {
	        ...
	        if (nodes && this.renderer.options.hydratable && !this.void) {
	            block.chunks.claim.push(b `${this.node.children.length > 0 ? nodes : children}.children.forEach(@detach);`);
	        }
	        if (renderer.options.dev) {
	...
	class HeadWrapper extends Wrapper {
	    ...
	    render(block, _parent_node, _parent_nodes) {
	        ...
	        if (nodes && this.renderer.options.hydratable) {
	            block.chunks.claim.push(b `${nodes}.children.forEach(@detach);`);
	        }
	    }
	}
	...
	class IfBlockWrapper extends Wrapper {
	    ...
	    render(block, parent_node, parent_nodes) {
	        ...
	        if (needs_anchor) {
	            block.add_element(anchor, x `@empty()`, parent_nodes && x `@claim_text(${parent_nodes}, '')/*IF242*/`, parent_node);
	        }
	        this.branches.forEach(branch => {
	            branch.fragment.render(branch.block, null, x `#nodes`);
	        });
	    ...
	    render(block, parent_node, parent_nodes) {
	        ...
	        if (can_use_innerhtml) {
	            ...
	            if (this.renderer.options.hydratable) {
	                ...
	                block.chunks.claim.push(b `${html_tag}.l(${parent_nodes});`);
	            }
	            if (needs_anchor) {
	...
	class Component {
	    ...
	    generate(result) {
	        ...
	        if (result) {
	            ...
	            const banner = `${this.file ? `${this.file} ` : ``}generated by Svelte v${'3.18.1'}`;
